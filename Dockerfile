...
COPY --chown=app:app lib/hyrax/autopopulation/version.rb ./lib/hyrax/autopopulation/version.rb
COPY --chown=app:app hyrax-autopopulation.gemspec ./hyrax-autopopulation.gemspec
...
RUN chmod 777 -R .bundle/* # Otherwise `app` owns this file and the host cannot run bundler commands
